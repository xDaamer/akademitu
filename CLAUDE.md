...
- `ALLOWED_ORIGINS` is comma-separated and enforced server-side in `server.ts`. It now matters for every request, since all Supabase traffic goes through `/api/*`. Since the split it must list **all three** production origins — apex, `www` and `portal` — or the missing one gets a 403; include `http://localhost:3000` for local development. Note it is not CSRF protection on its own (see the CSRF note in Architecture).
