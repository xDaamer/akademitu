...
Anti-spam: honeypot (`website`), Turkish mobile format validated on the server, a mandatory `Origin` header, the global `enforce_leads_insert_rate_limit` trigger (20 inserts/minute site-wide), and a **Postgres-backed per-IP limiter** (`public.auth_attempts`, kind `lead`, 5 per 10 minutes). The per-IP counter used to be an in-memory `Map` in `server.ts`; that limited nothing on Vercel, where each serverless instance has its own memory and an attacker resets the counter simply by landing on a new one. No CAPTCHA; reintroducing Cloudflare Turnstile was considered and declined.
